        ...
        private void UpdateSlaves(Quaternion q, double s, Vector3D t)
        {
            ...
            if (_slaves != null)
            {
                foreach (Viewport3D i in _slaves)
                {
                    ...
                    ModelVisual3D mv = i.Children[i.Children.Count-1] as ModelVisual3D;
                    Transform3DGroup t3dg = mv.Transform as Transform3DGroup;
                    if (t3dg == null)
                    {
                        t3dg = new Transform3DGroup();
                        t3dg.Children.Add(new ScaleTransform3D(1,1,1));
                        t3dg.Children.Add(new RotateTransform3D(new AxisAngleRotation3D(new Vector3D(0,1,0), 0)));
                        t3dg.Children.Add(new TranslateTransform3D(0,0,0));
                        mv.Transform = t3dg;
                    }
